...
> ## ⚠️ HEART-RATE DETECTION IS NOT VALIDATED
>
> The heart-rate code in this repository has **never been shown to detect a real human heartbeat from real CSI data**. Every heartbeat test feeds synthetic sine waves to the detector. The ONNX "model" path is dead code that silently falls back to a hardcoded 70 BPM heuristic. The project's own ADR-013 concedes that commodity WiFi has "micro-Doppler resolution insufficient for consistent heartbeat."
>
> **Any BPM value emitted by this software is unvalidated and must not be used for medical, safety, or legal purposes.** Read [HEARTBEAT_NOT_VALIDATED.md](HEARTBEAT_NOT_VALIDATED.md) for the full code review and the concrete list of things that would need to change to make the heart-rate modules real. Breathing detection (a separate code path) is not covered by this warning.

## **See through walls with WiFi** ##

**Turn ordinary WiFi into a sensing system.** Detect people, measure breathing, track movement, and monitor rooms — through walls, in the dark, with no cameras or wearables. Just physics. (Heart-rate sensing is present in the codebase but [not validated](HEARTBEAT_NOT_VALIDATED.md) — do not trust its output.)
